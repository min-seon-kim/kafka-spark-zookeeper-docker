...
class Cluster():
    ...
    def process(self):
        ...
        kmeans_obj.process()
        """
        xmeans_instance = xmeans(data=data, initial_centers=self.centroid_initializer, kmax=2000)
...
if __name__ == "__main__":
    ...
    for coord_weight in coord_weights:
        ...
        print("START xmeans_{}-weights".format(coord_weight))
        #data = fast_text_dict["normalized_concat_vec"]
        data = np.array(fast_text_dict["embedding_vec"], dtype=np.float64)
            
        x_means = Cluster(data,
                          amount_centers=2,
                          distance_metric="l2",
                          display=False)

        xmeans_result = x_means.process()

        # save
        with open('data/xmeans_weight-{}_data.pickle'.format(str(coord_weight)), 'wb') as f:
            pickle.dump(fast_text_dict, f)

        with open('data/xmeans_weight-{}_xmeans_object.pickle'.format(str(coord_weight)), 'wb') as f:
            pickle.dump(xmeans_result, f)
